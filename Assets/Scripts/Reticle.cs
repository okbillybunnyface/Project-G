...
public class Reticle : MonoBehaviour {
	...
	void Update () 
	{
		float deltaTimeIgnored = Time.realtimeSinceStartup - timeTracker;
		timeTracker = Time.realtimeSinceStartup;
		transform.position = Vector3.Lerp(transform.position, player.transform.position + new Vector3(direction.x * distance, direction.y * distance, 0), deltaTimeIgnored * smoothiness);
